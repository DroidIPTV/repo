...
def nuke():
    ...
    if not yes:
        return
    d.notification("Archiv", "Sicherung wird wieder hergestellt.")
    replace(url)
    d.notification("Archiv", "Archiv erfolgreich wieder hergestellt.")
...
def move_favourite_to_folder(favourites_file, name, url, thumbnail, fanart):
    ...
    if not where.startswith(top_folder):
        d.notification("Fehler", "Please keep to the folders path")
        return
        ...
        if url:
            ...
            if plugin.get_setting('edit') == 'true':
                if plugin.get_setting('add') == 'false':
                    context_items.append(("[COLOR aquamarine][B]%s[/B][/COLOR] " % 'Add Menu',
                                          'ActivateWindow(10001,"%s")' % (plugin.url_for('add', path=folder_path))))
                if plugin.get_setting('sort') == 'false':
                    context_items.append(("[COLOR aquamarine][B]%s[/B][/COLOR] " % 'Move', 'RunPlugin(%s)' % (
                        plugin.url_for(move_favourite, favourites_file=favourites_file, name=label, url=url))))
                context_items.append(("[COLOR aquamarine][B]%s[/B][/COLOR] " % 'Move to Folder', 'RunPlugin(%s)' % (
                ...
                context_items.append(("[COLOR aquamarine][B]%s[/B][/COLOR] " % 'Remove', 'RunPlugin(%s)' % (
                    plugin.url_for(remove_favourite, favourites_file=favourites_file, name=label, url=url))))
                context_items.append(("[COLOR aquamarine][B]%s[/B][/COLOR] " % 'Rename', 'RunPlugin(%s)' % (
                    plugin.url_for(rename_favourite, favourites_file=favourites_file, name=label, fav=fav))))
                context_items.append(("[COLOR aquamarine][B]%s[/B][/COLOR] " % 'Change Thumbnail', 'RunPlugin(%s)' % (
                    plugin.url_for(change_favourite_thumbnail, favourites_file=favourites_file, thumbnail=thumbnail,
                                   fav=fav))))
                context_items.append(("[COLOR aquamarine][B]%s[/B][/COLOR] " % 'Change Fanart', 'RunPlugin(%s)' % (
...
def add_favourites(path):
    ...
    for fav in favourites:
        ...
        if url:
            context_items = []
            context_items.append(("[COLOR aquamarine][B]%s[/B][/COLOR] " % 'Add', 'RunPlugin(%s)' % (
...
def add(path):
    ...
    items.append({'label': "[B]Archiv Eintrag[/B]", 'path': plugin.url_for('add_favourites', path=path),
                  'thumbnail': get_icon_path('favourites'), })

    items.append({'label': "Neuer Ordner", 'path': plugin.url_for('add_folder', path=path),
...
def index_of(path=None, silent=False):
    ...
    for folder in sorted(folders, key=lambda x: x.lower()):
        ...
        if plugin.get_setting('edit') == 'true':
            ...
            context_items.append(("[COLOR aquamarine][B]%s[/B][/COLOR] " % 'Remove',
                                  'RunPlugin(%s)' % (plugin.url_for(remove_folder, path=folder_path))))
            context_items.append(("[COLOR aquamarine][B]%s[/B][/COLOR] " % 'Rename',
                                  'RunPlugin(%s)' % (plugin.url_for(rename_folder, path=path, name=folder))))
            context_items.append(("[COLOR aquamarine][B]%s[/B][/COLOR] " % 'Change Thumbnail',
                                  'RunPlugin(%s)' % (plugin.url_for(change_folder_thumbnail, path=folder_path))))
            context_items.append(("[COLOR aquamarine][B]%s[/B][/COLOR] " % 'Change Fanart',
                                  'RunPlugin(%s)' % (plugin.url_for(change_folder_fanart, path=folder_path))))
            context_items.append(("[COLOR aquamarine][B]%s[/B][/COLOR] " % 'Change Colour',
                                  'RunPlugin(%s)' % (plugin.url_for(change_folder_colour, path=folder_path))))
            context_items.append(("[COLOR aquamarine][B]%s[/B][/COLOR] " % 'Set Password',
